    ...
    placements.forEach(p => {
      if (p.status === 'pending' || p.status === 'active') {
        ...
        if (startDays > 0 && startDays <= reminderDays) {
          events.push({ title: t('dashboard.eventStart', { name: fullName }), days: startDays, type: 'start', id: p.id + 's' });
        }
        if (endDays > 0 && endDays <= reminderDays) {
          events.push({ title: t('dashboard.eventEnd', { name: fullName }), days: endDays, type: 'end', id: p.id + 'e' });
        }
      }
    });
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500" onClick={() => setGlobalSearch('')}>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 relative z-30">
        <div>
          <h2 className="text-3xl font-bold text-zinc-900 tracking-tight">{t('nav.dashboard')}</h2>
          <p className="text-zinc-500 mt-2">{t('dashboard.desc')}</p>
        </div>
            ...
            <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-xl shadow-xl border border-zinc-200 overflow-hidden z-50">
              {searchResults.students.length > 0 && (
                <div className="p-2 border-b border-zinc-100">
                  <div className="text-xs font-semibold text-zinc-500 uppercase px-3 py-1 mb-1">{t('nav.students')}</div>
              ...
              {searchResults.companies.length > 0 && (
                <div className="p-2 border-b border-zinc-100">
                  <div className="text-xs font-semibold text-zinc-500 uppercase px-3 py-1 mb-1">{t('nav.companies')}</div>
              ...
              {searchResults.placements.length > 0 && (
                <div className="p-2">
                  <div className="text-xs font-semibold text-zinc-500 uppercase px-3 py-1 mb-1">{t('nav.placements')}</div>
              ...
              {searchResults.students.length === 0 && searchResults.companies.length === 0 && searchResults.placements.length === 0 && (
                <div className="p-4 text-center text-zinc-500">
                  {t('dashboard.noResults', { search: globalSearch })}
                </div>
              )}
            </div>
          )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <StatCard title={t('nav.students')} value={students.length} icon={Users} color="bg-blue-500" onDoubleClick={() => navigate('/students')} />
        <StatCard title={t('nav.companies')} value={companies.length} icon={Building2} color="bg-purple-500" onDoubleClick={() => navigate('/companies')} />
        <StatCard title={t('dashboard.statActive')} value={activePlacements} subtitle={t('dashboard.statPending', { count: pendingPlacements })} icon={Briefcase} color="bg-emerald-500" onDoubleClick={() => navigate('/placements')} />
      </div>
      ...
      <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden">
        <div className="px-6 py-5 border-b border-zinc-100 bg-zinc-50/50">
          <h3 className="text-lg font-semibold text-zinc-900 flex items-center">
            ...
            {t('dashboard.upcoming', { days: reminderDays })}
          </h3>
        </div>
            ...
            upcomingEvents.map(event => (
              <div key={event.id} className="p-4 px-6 flex items-center justify-between hover:bg-zinc-50 transition-colors">
                ...
                <span className="text-sm text-zinc-500 font-medium bg-zinc-100 px-3 py-1 rounded-full">
                  {event.days === 1 ? t('dashboard.daysRemaining.one', { days: event.days }) : t('dashboard.daysRemaining.other', { days: event.days })}
                </span>
              </div>
            ))
          )}
